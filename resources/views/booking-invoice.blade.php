...
<body class="bg-gray-100 min-h-screen">
	<div class="max-w-4xl mx-auto p-4">
		...
		<div class="no-print hidden md:flex justify-between items-center mb-6">
			...
			<div class="space-x-2">
				...
				@if (optional($booking)->status === 'pending')
	...
	<!-- Invoice Card -->
	<div class="bg-white rounded-lg shadow-lg overflow-hidden mb-20 md:mb-6 max-w-4xl mx-auto">
		<!-- Invoice Header -->
			...
			<div class="bg-gray-50 rounded-lg p-4 mb-6">
				...
				<div class="space-y-3">
					...
					<div class="flex flex-col md:flex-row justify-between items-start md:items-center">
						...
						<span class="text-blue-600 font-bold text-lg mt-1 md:mt-0">{{ $booking->formatted_total_price }}</span>
					</div>
				</div>
			</div>
	...
	<!-- Footer -->
	<div class="text-center text-gray-500 text-sm mt-6 no-print max-w-4xl mx-auto">
	...
	{{-- Tombol Bayar Mobile - Fixed (render terpisah agar tidak ter-clipping container) --}}
	@if (optional($booking)->status === 'pending')
		<div class="md:hidden fixed bottom-0 left-0 right-0 p-2 bg-white shadow-lg z-50 no-print">
	...
	{{-- Handler pembayaran (desktop & mobile) --}}
	@if (optional($booking)->status === 'pending')
	<script>
	(function () {
		...
		async function createPaymentAndRedirect(buttonEl) {
			try {
				setBusy(buttonEl, true, 'Memproses...');
				...
				if (!res.ok) {
					...
					setBusy(buttonEl, false, '💳 Bayar Sekarang');
					return;
				}
				const json = await res.json();
				if (json.success && json.redirect_url) {
					window.location.href = json.redirect_url; // VT-Web Snap
					return;
				}
				if (json.success && json.snap_token) {
					const base = {{ config('midtrans.is_production') ? "'https://app.midtrans.com'" : "'https://app.sandbox.midtrans.com'" }};
					window.location.href = base + '/snap/v2/vtweb/' + json.snap_token;
					return;
				}
		...
		document.addEventListener('DOMContentLoaded', function () {
			const btnDesktop = document.getElementById('payButton-desktop');
			const btnMobile = document.getElementById('payButton-mobile');
			if (btnDesktop) btnDesktop.addEventListener('click', () => createPaymentAndRedirect(btnDesktop));
			if (btnMobile) btnMobile.addEventListener('click', () => createPaymentAndRedirect(btnMobile));
		});
	})();
	</script>
	@endif
